-- Emails composed and sent from the Emails page share `mail_pixels` with pasted pixels,
-- so both appear in one list and are counted the same way. `sent_at` marks the ones
-- Worklane sent itself; there's no paste to ignore, so only the first seconds are skipped.

alter table public.mail_pixels
  add column if not exists sent_at timestamptz,
  add column if not exists body text check (char_length(body) <= 20000),
  add column if not exists cc text[] not null default '{}',
  add column if not exists message_id text;

create index if not exists mail_pixels_sender_sent_idx
  on public.mail_pixels (created_by, sent_at desc)
...
begin
  ...
  select p.id, p.organization_id, p.created_by, p.claim, p.subject, p.recipient, p.lead_id,
         p.self_ip_hashes, p.created_at, p.copied_at, p.sent_at
    into v_pixel
  ...
  if found then
    ...
    if (v_pixel.sent_at is not null and v_pixel.sent_at > now() - interval '10 seconds')
       or (v_pixel.sent_at is null
           and greatest(v_pixel.created_at, v_pixel.copied_at) > now() - interval '2 minutes')
